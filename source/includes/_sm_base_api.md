...
icon_url | M          | String | (X) 상단 타이틀에 보여질 로고 이미지 URL -> 로고 이미지는 현장 메인 정보 API 내 로고 이미지 이용 
...
현장관제 화면 표시를 위한 기본적인 정보 ( 로고 및 백그라운드 이미지/색상, 현장 및 UI 컴포넌트, 날씨 ) 를 제공합니다.

<aside class="notice">
사용자 인증 ( HTTP Bearer ) 필요 
</aside>

신규 API [Swagger](https://ras.hulandev.co.kr/imoa/swagger-ui/index.html#/%5B4.3%5D%20IMOS%20%ED%98%84%EC%9E%A5%EA%B4%80%EC%A0%9C%20%EB%A9%94%EC%9D%B8%20%EA%B5%AC%EC%84%B1%20%EC%A0%95%EB%B3%B4%20API%20/workplaceMainUsingPOST_1)


### HTTP Request

`POST /imoa/api/monitor/4.3/workplace/{wp_no}/main`
...
항목 | 필수 여부(M/O) | 데이터 타입 | 설명
--------- |------------| -----------| -----------
deploy_page | O          | number | 화면 페이지 ( 디폴트값 1  )
...
bg_img_url | O          | string              | 백그라운드 이미지 URL
bg_color  | O          | string              | 백그라운드 색상 ( 백그라운드 이미지 없을 경우 적용 )
weather | M          | WeatherInfo         | 현장 날씨 정보
ui_component_list | M          | List<UiComponent>   | 설정된 UI 컴포넌트 리스트
monitoring_workplace | M          | WorkplaceInfo | 현장 정보. 현장 정보 상세는 [WorkplaceInfo](#workplaceinfo) 참고
btn_list | O          | List   | 외부 링크 리스트
wp_data_list | O          | List<WpData>        | UI 컴포넌트별 현장 부가 정보 리스트 ( 컴포넌트별로 custom_data 지정 사용 ) 
...
항목 | 필수 여부(M/O) | 데이터 타입              | 설명
--------- |------------|---------------------| -----------
btn_name | M          | string              | 링크명
btn_link_url | O          | string              | 링크 URL ( 새창 띄우기 )
